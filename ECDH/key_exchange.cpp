...
int main(int argc, char** argv) {
    ...
    // get other pubKey
    string px = argv[2], py = argv[3];
    Integer Px(px.c_str()), Py(py.c_str());
    ECP::Point P(Px, Py);

    // calculate shared_key using privKey and other pubKey
    ECP::Point S = curve256.GetCurve().ScalarMultiply(P, d);
    cout << S.x << endl;
    return 0;
}